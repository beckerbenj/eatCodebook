...
Now comes the harder part of actually creating and editing the `varinfo`. You can either create and save a `varinfo.xlsx` file in `0_main.R` or you open `2_varinfo.R` and create `varinfo.xlsx` here. There's a lot to add,  While working on the `varinfo` you often need to recreate it multiple times, so I recommend writing a script in the additional file `2_varinfo.R` to do the editing. Then you can just rerun it without having to think too much. You should check it after each recreation, though, whether the script still worked correctly. 

When working on the BT codebook for IQB, you can find most Excel file containing the additional information to add on the network server *FDZ und Bildungstrend*. 
