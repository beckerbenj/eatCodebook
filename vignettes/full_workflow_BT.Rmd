...
Some information may need to be modified because the function does not label it correctly. For this, it is necessary to understand the functionality and check the variable entries.
...
The **scale** column specifies how the variable is to be displayed. If it is empty, no descriptives are displayed. *numeric* is a variable without labeled categories. For *ordinal*, it is a categorical variable that has labeled values. *nominal*  (**add examples??**)
...
The BT often uses imputations for the student data set (sus). You can recognize them by the add-on `imputiert` in their label. 
