...
In the **Gliederung** (subsection numbers) column, the subsection numbers must be inserted. Subsections such as "1.1", "1.2",... "2.1". The names of the corresponding sections is done in a later function (s. [7. Structure](#7.-structure)). Make sure the subsections names and numbers match. 
...
The *intro* to the codebook usually has separate references that you need to add as well.
...
# find duplicates
anyDuplicated(lit_long$Kurzangabe)
# identify duplicates
pos <- duplicated(lit_long$Kurzangabe)
dup <- lit_long$Kurzangabe[pos]
# look at (first) duplicated entry
lit_long[lit_long$Kurzangabe == dup,]
```

After identifying the duplicates you can look at them manually to find out where the difference in the long form references come from. If it's minor differences, you can just delete the extra ones. Sometimes the URL is missing, so you should delete the reference(s) with the least amount of info.

```{r lit long remove duplicates, eval=FALSE}
# delete all duplicates
lit_long <- lit_long[!pos,]
# delete specific rows example
View(lit_long)
lit_long <- lit_long[-21,]
```

Then you test once more, if there are any duplicates left, the output should be `0`. 

```{r lit long duplicates 2, eval=FALSE}
anyDuplicated(View(lit_long)$Kurzangabe)
```
### Match Long and Short References

Now you have two data frames: **literatur** contains all short form references actually used in `varinfo` and **lit_long** containing the long form references. Now you need to transfer the long form references to **literatur**.

```{r lit transfer long, eval=FALSE}
pos <- match(literatur$Kurzangabe, lit_long$Kurzangabe)
literatur$Langangabe <- lit_long$Langangabe[pos]
View(literatur)
```

### Clean Up Table

After you transferred all the references correctly, you still need to add which references should be displayed in the codebook and change the order to be alphabetical. Every entry with a long form reference should be displayed and set to `ja`. 
...
There are two more optional files you can add the the `codebook()` function: *intro* and *lastpage*. *intro* contains the intro text plus all the formatting embedded in LaTeX code. *lastpage* contains information about the bibiography something something. 
...
You will probably have to create multiple codebook versions depending you progress and to check if variables and chapters or other additions are displayed the way you want it to. There are arguments the function `codebook()` needs to create a minimal version of a codebook. And additions that are not necessary, but that you need for the BT codebook, like a bibiography, a cover or an intro. 
...
The information from `gliederung` and `chapters` will create a **table of content** for the codebook. Everything else are optional additions. 
...
You can add a **bibliography** with all references that you saved to the Excel `literatur.xlsx`.
