...
The appendix should look like this: `Hintergrundvariable` contains the variable, `Erstellt aus` the variable(s) the (new) variable was created from (if any), and `Inhalt der Hintergrundvariable` contains the variables label, e.g. the content of the background variable. 
...
There are two more optional files you can add the the `codebook()` function: *intro* and *lastpage*. **intro** contains the intro text plus all the formatting embedded in LaTeX code. **lastpage** contains the series of publications.
...
You open the folder of last year's BT on Q: and go to a folder names *Texte* or *08_Einleitungstexte*. You copy the file `Latex_lastpage.tex` or `Latex_lastpage_snippet.tex` into your work space in the folder *Texte*. 
