...
| `4_literatur.R` | Create and edit the `reference` file. You have to match citations to their respective references. You also have to add proper LaTeX syntax for italic text or URLs. |
...
The function `createScaleInfo()` adds imputed variables like a scale to the data frame. Without the scale variable the information which variables belong together might be lost. In this case, it might not be relevant, though, as these variables are not a scale.
...
In **VarName** you have the name of the scale variable or the group name (they should be the same). **Items_der_Skala** contains all items belonging to the scale or which have the same group name as a scale variable. 
...
Not every variable in the file might be a scale (see [Imputations](#imputation-variables). All errors and mismatches occurring here can or should usually be fixed in the `inputForDescriptives` files. 
