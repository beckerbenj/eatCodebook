...
Not every variable will have a reference or instructions, but NAs, so if you take the column `Quelle` on its own, it should have the same row length as the `varinfo`. Either way, you need to check whether the variables are the same in both data frames. `match()` needs to character vectors. The function compares whether the names in each vector are the same and the order they're in. The output should be a numerical vector with numbers from 1 to the number of rows in `quellen_sus$Variable`. If the numbers are switched or you get `NA`s, the variable names don't match and you need to find out why that is. 

```{r varinfo reference match var, eval=FALSE}
match(quellen_sus$Variable, varinfo$sus$Var.Name)
```

The order of the variables should match, but doesn't necessarily. It might be that there are variables in `quellen_sus` that are not in `varinfo$sus`. You would have to remove them from the data frame. If that doesn't solve the problem, something else might be wrong, in which case you should ask your supervisor or whoever is responsible for the data sets. 

#### Remove and Add Rows

You identify the name of the variable that is in `quellen_sus` but not in `varinfo$sus` with `View()`, get the position with `grep()` and remove the line(s).

```{r varinfo ref remove rows, eval=FALSE}
# identify position of variable "IDBL"
pos <- grep("IDBL", `quellen_sus$Variable`)
# remove line
quellen_sus <- quellen_sus[-pos,]
```

#### Match Order and Write references/instructions in Varinfo

After making sure you have the right variables in your `quellen` object, you should match the order again (to make sure). In `pos` you should now have the variable's order (as a numeric vector) of the variable names from `varinfo$sus` as they appear in `quellen_sus`.  

```{r varinfo reference match var 2, eval=FALSE}
pos <- match(quellen_sus$Variable, varinfo$sus$Var.Name)
```

With `varinfo$sus$QuelleSH[pos]` you now have the variable names (and the rest of the columns) in the same order as `quellen_sus`, so you can just add the character vectors with the references and instructions (`quellen_sus$Quelle` and `quellen_sus$Instruktion`) in the corresponding column in `varinfo$sus`. 
...
It's best to compare the two data frames once more to make sure the references and instructions are matched with the correct variable. You can check all or a random set of variables (1:10) and look at the columns ins question. 
...
When everything matched correcty, save your progress and import the Excel with `getVarInfo()`.
...
#### seperate structure and subsection names
