...
test_that("descriptives pooled ordinal", {
  imputed_cols <- data.frame(imp1 = c(1, 2, 1, 2), imp2 = c(2, 2, 1, 1))
  value_table <- data.frame(varName = "Var1",
                            value = c(1, 2, 3),
                            missings = c("valid", "valid", "valid"),
                            stringsAsFactors = FALSE)
  warns <- capture_warnings(out <- kennwerte.gepoolt.ordinal(imputed_cols, imputedVariableCols = c("imp1", "imp2"),
  ...
  expect_equal(out[["N.valid"]], "4")
  expect_equal(out[["mean.valid"]], "1.50")
  expect_equal(out[["1.valid"]], "50.0")
  expect_equal(out[["2.valid"]], "50.0")
  expect_equal(out[["3.valid"]], "0.0")
  expect_equal(out[["1.totalabs"]], "2")
  expect_equal(out[["sysmis.totalabs"]], "0")
  expect_equal(names(out), c("N.valid", "N.total", "mean.valid", "sd.valid",
                             "1.valid", "2.valid", "3.valid", "sysmis.valid",
  ...
  suppressWarnings(out2 <- kennwerte.gepoolt.ordinal(imputed_cols, imputedVariableCols = c("imp1", "imp2"),
                                                        value_table = value_table, verbose = FALSE))
  ...
  expect_equal(names(out2), c("N.valid", "N.total", "mean.valid", "sd.valid",
                              "1.valid", "2.valid", "3.valid", "sysmis.valid",
                              "1.total", "2.total", "3.total", "sysmis.total",
                              "1.totalabs", "2.totalabs", "3.totalabs", "sysmis.totalabs"))
})
